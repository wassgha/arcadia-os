...
local LauncherScreen = setmetatable({}, {
    __index = Screen
})
LauncherScreen.__index = LauncherScreen

local SCROLL_SPEED = 10

function LauncherScreen:new()
...
function LauncherScreen:update(dt)
    local itemWidth = math.floor((self.screenWidth - self.gap) / self.cols) - self.gap
    local itemHeight = itemWidth * (2 / 3)
...
function LauncherScreen:load()
    ...
    arcadia.controls:on(function(key)
        ...
        elseif key == 'A' then
            if self.focused == 1 then
                arcadia.navigation:switchTo("App", {
                    appName = 'clock'
                })
